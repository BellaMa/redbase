...
RC IX_Manager::CreateIndex(const char *fileName, int indexNo,
    ...
    //We only want to apply R-tree, the only attrType taking is MBR
    if(attrType != _MBR)
        return (IX_BADINDEXSPEC);
    // Create index file:
...
bool IX_Manager::IsValidIndex(AttrType attrType, int attrLength){
    if(attrType == INT && attrLength == 4)
        ...
        return true;
    else if(attrType == _MBR && attrLength == sizeof(MBR))
        return true;
    else
        return false;
}
...
RC IX_Manager::SetUpIH(IX_IndexHandle &ih, PF_FileHandle &fh, struct IX_IndexHeader *header){
    ...
    else if(ih.header.attr_type == _MBR){
        ih.comparator = compare_mbr;
        ih.printer = print_mbr;
    }
    else{
